...
def run_trading_session(api, strategy, pairs, cfg, memory, ai_analyzer):
    ...
    # UI Loop (usando o dashboard já criado acima)
    last_ui_update = time.time()
    
    try:
        with Live(dashboard.render(current_profit), refresh_per_second=4) as live:
            while not stop_threads:
                # Watchdog Check
                now = time.time()
                
                # Se o worker travar, ele nao atualiza variaveis, mas aqui podemos
                # monitorar se o timestamp do ultimo log mudou ou apenas avisar
                # se a UI travar (o que é raro).
                # O melhor watchdog é verificar se a ultima mensagem do log é muito antiga
                # se "Iniciando" já passou.
                ...
                live.update(dashboard.render(current_profit, secs))
                time.sleep(0.25)
    ...
    if Prompt.ask("  🤖 [bright_white]Ativar IA?[/bright_white]", choices=["s", "n"], default="s") == "s":
        try:
            with Progress(
                ...
                key = "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"
